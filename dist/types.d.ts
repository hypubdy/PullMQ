...
export type ConnectionOptions = Redis | {
    host?: string;
    port?: number;
    password?: string;
    db?: number;
...
export interface JobsOptions extends DefaultJobOptions {
    ...
    /** PullMQ extension: group-based scheduling */
    group?: GroupOptions;
}
...
export type JobState = 'completed' | 'failed' | 'active' | 'delayed' | 'waiting' | 'paused' | 'waiting-children' | 'prioritized' | 'unknown';
export type FinishedStatus = 'completed' | 'failed';
export type JobProgress = number | object;
export type Processor<T = unknown, R = unknown, N extends string = string> = (job: import('./job').Job<T, R, N>, token?: string, signal?: AbortSignal) => Promise<R>;
